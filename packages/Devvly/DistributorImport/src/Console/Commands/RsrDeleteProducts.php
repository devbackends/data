        ...
        foreach ($rsrProducts as $rsrProduct){
            if($rsrProduct[2]=="DELETED"){
                ...
                if (isset($result[0])) {
                    ...
                    $upc=$data['upcCode'];
                    if($upc){
                        $deletedProduct=app('Devvly\DistributorImport\Services\Product')->deleteRsrProduct($upc);
                        if(isset($deletedProduct['status'])){
                            if($deletedProduct['status']=='success'){
                              app("Devvly\Product\Repositories\ProductRepository")->sendDeleteProductWebhook($upc,'rsr');
                              Log::info('Product with upc '.$upc.' has been deleted');
                            }
                        }
                    }
                }
