...
class UpdateDistribute extends Command
{
    ...
    protected $signature = 'update-zanders-distribute';

    protected $description = 'Update products distributor feed';

    public function handle()
    {
        $checkProductsImported=DB::table('zanders_logs')->select('*')->where('id', 1)->get();
        if(isset($checkProductsImported[0])){
            $log=$checkProductsImported[0];
            if($log->import_zanders==1){
                // Get data
                $this->setUpdateLog(0);
                $this->comment('Update | Start Updating');
                $call = $this->call('update-zanders');
                // Call next commands
